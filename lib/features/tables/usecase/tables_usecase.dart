...
class TableGenUsecase extends TableGenRepo {
  ...
  Future<List<TablesModel>> saveTables(List<TablesModel> tables) async {
    try {
      ...
      await db.collection('tables').remove({'year': tables.first.year, 'semester': tables.first.semester});
      await db.collection('tables').insertAll(tables.map((e) => e.toMap()).toList());
      return tables;
    } catch (e) {
      return [];
    }
  ...
  Future<(TablesModel?, TablesModel?, String)> swapTables(
    ...
    try {
     if(db.state != State.open){
      ...
      await db.collection('tables').update({'id': table2
      .id}, table2.toMap());
  ...
  Future<(TablesModel?, String)> updateItem(TablesModel newTable) async {
    try {
      ...
      await db.collection('tables').update({
        'id': newTable.id
      }, newTable.toMap());
